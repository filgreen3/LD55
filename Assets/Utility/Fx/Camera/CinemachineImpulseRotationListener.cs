...
namespace Cinemachine
{
    ...
    public class CinemachineImpulseRotationListener : CinemachineExtension
    {
        /// <summary>
        ...
        public struct ImpulseReaction
        {
                ...
                out Vector3 pos, out Quaternion rot)
            {
                ...
                rot = Quaternion.Euler(NoiseSettings.GetCombinedFilterResults(
                        m_SecondaryNoise.OrientationNoise, m_CurrentTime, m_NoiseOffsets) * gain * (UnityEngine.Random.value * 2 - 1f));
            ...
            CinemachineCore.Stage stage, ref CameraState state, float deltaTime)
        {
            if (stage == m_ApplyAfter && deltaTime >= 0)
            {
                ...
                if (haveImpulse || haveReaction)
                {
                    ...
                    // Debug.Log();
                    Vector3 rotCorrect = state.OrientationCorrection.eulerAngles;
                    rotCorrect.x += impulsePos.y;
                    rotCorrect.y += impulsePos.x;
                    rotCorrect.z += impulsePos.z;

                    state.OrientationCorrection = Quaternion.Euler(rotCorrect);
                }
            }
        }
    }
